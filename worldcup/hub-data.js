...
window.useHubStore = function () {
  ...
  const removePlayer = (id) => { setCollections((c) => { const n = Object.assign({}, c); delete n[id]; return n; }); setPlayers((p) => { if (p.list.length <= 1) return p; try { localStorage.removeItem(bkey(id)); localStorage.removeItem(skey(id)); } catch (e) {} const list = p.list.filter((x) => x.id !== id); return { list: list, active: p.active === id ? list[0].id : p.active }; }); };
